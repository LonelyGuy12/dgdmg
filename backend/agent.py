...
async def run_agent(nl_request: str) -> AsyncGenerator[dict, None]:
    ...
    try:
        ...
        if not github_token or not github_repo:
            ...
            # NOTE: do NOT return here — fall through to Step 8 (DataHub Write-Back)
        else:
            github = GitHubClient()
        ...
        # Register the generated model and its upstream lineage back in DataHub.
        # This step is non-fatal: any error emits a "warning" event so the
        # existing pipeline steps 1-7 are never affected.
        # ─────────────────────────────────────────────────────────────────────
        yield _event(
            "datahub_writeback",
            "running",
            f"Registering `{model_name}` in DataHub catalog and adding lineage…",
        )

        try:
            # Build a normalised dataset record from the pipeline variables
            # that were finalised in Steps 5 & 6.
            dataset_fields = [
                {
                    "name":         col.get("name", ""),
                    "type":         "VARCHAR",  # default; LLM doesn't always supply type
                    "description":  col.get("description", ""),
                    "isPrimaryKey": col.get("isPrimaryKey", False),
                    "isForeignKey": col.get("isForeignKey", False),
                    "fkTable":      col.get("fkTable"),
                    "fkColumn":     col.get("fkColumn"),
                    "tags":         col.get("tags", []),
                }
            ...
            reg_result = await mcp.register_dataset(dataset_record)

            # Discover upstream source tables from the generated SQL
            source_tables = extract_source_tables(sql_content)

            lineage_results = []
            for source in source_tables:
                edge_result = await mcp.add_lineage(
                    upstream_name=source,
                    downstream_name=model_name,
                    transformation_type="DBT_MODEL",
                    transformation_query=sql_content[:4000],
                )
                lineage_results.append({
                    "upstream": source,
                    "added":    edge_result.get("added", False),
                    "duplicate": edge_result.get("duplicate", False),
                })
            ...
            yield _event(
                ...
                "done",
                (
                    f"Registered `{model_name}` in DataHub "
                    f"({len(lineage_results)} lineage edge(s) added)"
                ),
                {
                    "urn":          reg_result.get("urn", ""),
                    "name":         model_name,
                    "lineageEdges": lineage_results,
            ...
            yield _event(
                "datahub_writeback",
                "warning",
                f"DataHub write-back failed (non-fatal): {wb_err}",
